...
if(isset($_POST['getUlikes']) && isset($_POST['userid']) && is_numeric($_POST['userid'])) {

	extract($_POST);

	

	$ex_userLikesTaste = ex_findall("ex_likes", "WHERE uID='$userid' AND lTYPE='1' ORDER BY ID DESC LIMIT $artworkLikeStart, $artworkLimit");

	if(count($ex_userLikesTaste) > 0) {

	foreach ($ex_userLikesTaste as $likeItem) {
		...
		$logeduseralreadyliked = ex_cellcount("ex_likes", "aID", $likeItem->aID, "AND uID='" . exAuth_getsession("userid") . "' AND lTYPE='1'");

		$uif = ex_findone("ex_users", "ID", $userid);
			...
			<div class="exart-inner">					
				...
				<div class="exart-content">
					...
					<div class="exart-art" style="/*background-color: #<?= ex_randomcolor() ?>*/">
						...
						<!-- Like -->
						<?php if($uif->username !== exAuth_getsession("username")): ?>
